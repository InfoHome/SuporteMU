...
/****** Object:  StoredProcedure [dbo].[CARGA_BI_CATEGORIA_REDE]    Script Date: 01/06/2018 08:27:35 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[CARGA_BI_CATEGORIA_REDE]            
...
SELECT   
    ...
    SUM(PROD.PRECOCOMP*ITPD.QUANT),            
         0,0,0            
FROM   
	BDENTER.DBO.VENDASSCAD VEN,            
    BDENTER.DBO.PEDICLICAD PED,    
    BDENTER.DBO.ITEMCLICAD ITPD,             
    BDENTER.DBO.PRODUTOCAD PROD,
	BDENTER.DBO.AC_GC_EXT_CLASPROD CLAS
WHERE  
	CLAS.CLASPROD = SUBSTRING(PROD.CLASPROD,1,12)
	AND VEN.DTVEN >= '20140101'   
	AND VEN.NUMPED > 7   
...
INSERT INTO NIVEL7_AC_REDE            
SELECT   
	RIGHT('00000' + LTRIM(RTRIM( ITNF.CODPRO )),5),            
    SUBSTRING(CONVERT(VARCHAR,VEN.DTVEN,103) ,7,4) + SUBSTRING(CONVERT(VARCHAR,VEN.DTVEN,103),4,2) + SUBSTRING(CONVERT(VARCHAR,VEN.DTVEN,103) ,1,2),            
    NF.FILIAL,
	CLAS.DESCR, -- Categoria
	--SUBSTRING(PROD.CLASPROD,1,12),        
...
WHERE  
	...
	AND VEN.DTVEN >= '20140101' 
	AND VEN.NUMPED <= 7 
	AND VEN.NUMORD = NF.NORDVEN   
    AND NF.ATUALIZ = 1 
	AND NF.EST IN (2,3)            
    AND LEFT(NF.TPO,1) = '2' 
	AND NF.NUMORD = ITNF.NUMORD    
    AND ITNF.CODPRO= PROD.CODPRO 
...
FROM  
	BDENTER.DBO.NFENTRACAD NE, 
	BDENTER.DBO.ITNFENTCAD ITNE, 
	BDENTER.DBO.NFSAIDACAD NF,            
    BDENTER.DBO.PRODUTOCAD PROD, 
	BDENTER.DBO.VENDEDOCAD VEN,    
	BDENTER.DBO.AC_GC_EXT_CLASPROD CLAS
WHERE  
	CLAS.CLASPROD = SUBSTRING(PROD.CLASPROD,1,12)
	AND NE.DTCHEG >= '20140101' 
	AND NE.ATUALIZ = 1 
	AND ITNE.CODVEND = VEN.CODVEND 
	AND NE.NUMORD = ITNE.NUMORD 
	AND ITNE.NUMORDDEV = NF.NUMORD    
    AND NE.EST IN (1,2,3) 
...
BEGIN TRANSACTION            
INSERT INTO tab_nivel3_AC_REDE (NIVEL3, DESCRICAO) SELECT DISTINCT NIVEL3, NIVEL3             -- CATEGORIA
                          FROM nivel3_AC_REDE            
...
BEGIN TRANSACTION            
INSERT INTO tab_nivel4_AC_REDE (NIVEL4, DESCRICAO) SELECT DISTINCT NIVEL4, 'VENDEDOR ' + NIVEL4            
                          FROM nivel4_AC_REDE            
...
BEGIN TRANSACTION            
INSERT INTO tab_nivel5_AC_REDE (NIVEL5, DESCRICAO) SELECT DISTINCT NIVEL5, 'FORNECEDOR  ' + NIVEL5            
                          FROM nivel5_AC_REDE            
...
BEGIN TRANSACTION            
INSERT INTO tab_NIVEL6_AC_REDE (NIVEL6, DESCRICAO) SELECT DISTINCT NIVEL6, 'CLIENTE ' + NIVEL6            
                          FROM NIVEL6_AC_REDE            
...
--- Categoria -------------------
--BEGIN TRANSACTION            
--UPDATE TAB_NIVEL3_AC_REDE 
--	SET DESCRICAO = LEFT(upper(DESCR),100)  
--		FROM TAB_NIVEL3_AC_REDE, BDENTER.DBO.AC_GC_EXT_CLASPROD      
--		WHERE NIVEL3 =  BDENTER.DBO.AC_GC_EXT_CLASPROD.Descr            
--COMMIT

--- Vendedor -------------------            
BEGIN TRANSACTION            
UPDATE TAB_NIVEL4_AC_REDE SET DESCRICAO =  upper(V.NOME),             
             GERENTE = V.CODGER            
           FROM TAB_NIVEL4_AC_REDE, BDENTER.DBO.VENDEDOCAD V            
           WHERE NIVEL4 = RIGHT('00000' + LTRIM(RTRIM( V.CODVEND )),5)            
COMMIT            

--- Gerente -------------------            
BEGIN TRANSACTION            
UPDATE TAB_NIVEL4_AC_REDE SET GERENTE = V.nome            
           FROM TAB_NIVEL4_AC_REDE, BDENTER.DBO.VENDEDOCAD V            
...
-- Cliente -----------------------  
BEGIN TRANSACTION            
UPDATE TAB_NIVEL6_AC_REDE SET DESCRICAO =  LEFT(upper(C.NOME),100)            
           FROM TAB_NIVEL6_AC_REDE, BDENTER.DBO.ITEM C            
           WHERE CAST(NIVEL6 AS INT) = C.OID            
COMMIT            
